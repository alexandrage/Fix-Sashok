...
public class eURLClassLoader extends URLClassLoader implements Closeable {
	 ...
	 *             an IOException. Any such exceptions are caught internally. If
	 *             only one is caught, then it is re-thrown. If more than one
	 *             exception is caught, then the second and following exceptions
	 *             are added as suppressed exceptions of the first one caught,
	...
	public static eURLClassLoader newInstance(final URL[] urls, final ClassLoader parent) {
		...
		eURLClassLoader ucl = AccessController.doPrivileged(new PrivilegedAction<eURLClassLoader>() {

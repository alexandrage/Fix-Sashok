...
public class eURLClassPath {
		...
		Resource getResource(final String name, boolean check) {
			...
			try {
				...
				if (uc instanceof JarURLConnection) {
					/*
					 * Need to remember the jar file so it can be closed in a
					 * hurry.
					 */
	...
	static class JarLoader extends Loader {
					...
					java.security.AccessController.doPrivileged(new java.security.PrivilegedExceptionAction<Void>() {
						public Void run() throws IOException {
							...
							if (index != null) {
								...
								// Add all the dependent URLs to the lmap so
								// that loaders
								// will not be created for them by
								// URLClassPath.getLoader(int)
								// if the same URL occurs later on the main
								// class path. We set
								// Loader to null here to avoid creating a
								// Loader for each
								// URL until we actually need to try to load
								// something from them.
								for (int i = 0; i < jarfiles.length; i++) {
									try {
										URL jarURL = new URL(csu, jarfiles[i]);
										// If a non-null loader already exists,
										// leave it alone.
		...
		URL[] getClassPath() throws IOException {
			...
			if (SharedSecrets.javaUtilJarAccess().jarFileHasClassPathAttribute(jar)) { // Only
																						// get
																						// manifest
																						// when
																						// necessary

...
public class ImageUtils {
	...
	public static BufferedImage genPanel(int w, int h, BufferedImage img) {
		...
		res.getGraphics().drawImage(img.getSubimage(onew, oneh, onew * 2, oneh * 2), w - onew, h - oneh, onew, oneh,
				null);

		try {
			res.getGraphics().drawImage(fill(img.getSubimage(onew, 0, onew, oneh), w - onew * 2, oneh), onew, 0,
					w - onew * 2, oneh, null);
		} catch (Exception e) {
		}
		try {
			res.getGraphics().drawImage(fill(img.getSubimage(0, oneh, onew, oneh), onew, h - oneh * 2), 0, oneh, onew,
					h - oneh * 2, null);
		} catch (Exception e) {
		}
		try {
			res.getGraphics().drawImage(fill(img.getSubimage(onew, oneh * 2, onew, oneh), w - onew * 2, oneh), onew,
					h - oneh, w - onew * 2, oneh, null);
		} catch (Exception e) {
		}
		try {
			res.getGraphics().drawImage(fill(img.getSubimage(onew * 2, oneh, onew, oneh), onew, h - oneh * 2), w - onew,
					oneh, onew, h - oneh * 2, null);
		} catch (Exception e) {
		}
		try {
			res.getGraphics().drawImage(fill(img.getSubimage(onew, oneh, onew, oneh), w - onew * 2, h - oneh * 2), onew,
					oneh, w - onew * 2, h - oneh * 2, null);
		} catch (Exception e) {
		}
	...
	public static BufferedImage fillHoriz(BufferedImage texture, int w, int h) {
		...
		BufferedImage img = new BufferedImage(w, h, 2);
		for (int x = 0; x <= w / sizex; x++)
			img.getGraphics().drawImage(texture, x * sizex, 0, sizex, texture.getHeight(), null);
		return img;
	}
	...
	public static BufferedImage parseSkin(BufferedImage skinIMG) {
		...
		g.drawImage(skinIMG.getSubimage(w * 44, h * 20, w * 4, h * 12), 0, 64, 32, 96, null);// Левая
																								// рука
		g.drawImage(skinIMG.getSubimage(w * 44, h * 20, w * 4, h * 12), 96, 64, 32, 96, null);// Правая
																								// рука
		g.drawImage(skinIMG.getSubimage(w * 4, h * 20, w * 4, h * 12), 32, 160, 32, 96, null);// Левая
																								// нога
		g.drawImage(skinIMG.getSubimage(w * 4, h * 20, w * 4, h * 12), 64, 160, 32, 96, null);// Правая
																								// нога

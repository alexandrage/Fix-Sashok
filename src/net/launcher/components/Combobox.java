...
public class Combobox extends JComponent implements MouseListener, MouseMotionListener {
	...
	public void paintComponent(Graphics gmain) {
		...
		int w = getWidth();
		if (pressed) {
			g.drawImage(genButton(w, pressedTX.getHeight(), pressedTX), 0, getHeight() - pressedTX.getHeight(), w,
					pressedTX.getHeight(), null);
			...
			g.drawImage(genPanel(w, getHeight() - pressedTX.getHeight(), panelTX), 0, 0, w,
					getHeight() - pressedTX.getHeight(), null);
			if (entered && y / pressedTX.getHeight() < elements.length) {
				g.drawImage(genButton(w, selectedTX.getHeight(), selectedTX), 0,
						y / pressedTX.getHeight() * pressedTX.getHeight(), w, pressedTX.getHeight(), null);
			}
			...
			g.drawString(elements[selected], 5,
					selectedTX.getHeight() * (elements.length + 1) - (g.getFontMetrics().getHeight() / 2));
		} else if (entered) {

...
public class Frame extends JFrame implements ActionListener, FocusListener {
	...
	public Frame() {
		try {
			...
			JOptionPane.showMessageDialog(Frame, "Запуск второй копии лаунчера невозможен!", "Лаунчер уже запущен",
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == login || e.getSource() == password || e.getSource() == toGame || e.getSource() == toAuth
				|| e.getSource() == toPersonal || e.getSource() == toGamePersonal) {
	...
	public void focusGained(FocusEvent e) {
		if (e.getSource() == login && login.getText().equals(Message.Login))
			login.setText(empty);
                if (e.getSource() == password &&password.getText().equals(Message.Password)){
	...
	public void focusLost(FocusEvent e) {
		if (e.getSource() == login && login.getText().equals(empty))
			login.setText(Message.Login); 
                if (e.getSource() == password &&password.getText().equals(empty)){
                    password.setText(Message.Password);
                    password.setEchoChar((char)0);
                }
	}

...
public class DragbuttonStyle {
	...
	public DragbuttonStyle(int hx, int hy, int hw, int hh, int cx, int cy, int cw, int ch, String texture,
			boolean visible) {
		this.hx = hx;
		this.hy = hy;
		this.hw = hw;
	...
	public void apply(Dragbutton hide, Dragbutton close) {
		...
		close.img2 = texture.getSubimage(texture.getWidth() / 3, texture.getHeight() / 2, texture.getWidth() / 3,
				texture.getHeight() / 2);
		close.img3 = texture.getSubimage(texture.getWidth() / 3 * 2, texture.getHeight() / 2, texture.getWidth() / 3,
				texture.getHeight() / 2);
	}
}